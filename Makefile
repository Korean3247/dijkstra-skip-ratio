...
.PHONY: reproduce-all reproduce-theory reproduce-bootstrap reproduce-sensitivity \
        reproduce-heap-py reproduce-heap-cpp reproduce-tables pdf clean-results help
...
## §III: Bootstrap 95% CI for α and c (requires theory output)
##   Output: resultsTheory/bootstrap_ci_*.csv
reproduce-bootstrap: resultsTheory
...
## Check that all result directories exist (created by experiments)
resultsTheory:
	@test -d resultsTheory || (echo "Run 'make reproduce-theory' first"; exit 1)
...
help:
	...
	@echo "============================================"
	@echo ""
	@echo "  make reproduce-all        Run all experiments in order"
	@echo "  make reproduce-theory     §III–IV: power-law fits (~10-20 min)"
	@echo "  make reproduce-bootstrap  §III: bootstrap CI (needs theory output)"
	@echo "  make reproduce-sensitivity §VII: source-vertex sensitivity (~2 min)"
	@echo "  make reproduce-heap-py    §VII: Python heap comparison (~5-10 min)"
	@echo "  make reproduce-heap-cpp   §VII: C++ heap comparison (~2 min)"
	@echo "  make reproduce-tables     §V–VI: timing experiments"
	...
	@echo "  C++ requires:  g++ -O2 -std=c++17 (expA also needs OpenMP)"
	@echo ""
