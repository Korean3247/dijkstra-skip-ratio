...
\begin{tabular}{p{2.2cm}p{1.7cm}p{3.4cm}}
\toprule
...
\addlinespace
Float weights, $k > 10$ (dense, C++) &
  Indexed or 4-ary heap &
  C++ indexed heap $1.4$--$2.2\times$ faster than lazy binary for $k\!\geq\!8$;
  4-ary heap $1.4$--$1.6\times$ faster (see §\ref{sec:heapcompare}). \\
\addlinespace
Float weights, $k > 10$ (dense, Python) &
  heapq (lazy deletion) &
  Fibonacci and indexed heap do not outperform heapq in Python; lazy wins. \\
\addlinespace
Fibonacci heap &
  Avoid in practice &
...
the lazy binary heap by $1.4$--$1.6\times$ ($V=10{,}000$, $k=4$:
0.76\,ms vs.\ 1.22\,ms; $k=128$: 3.95\,ms vs.\ 5.61\,ms); the reduced
tree height offsets the wider sift-down scan once interpreted-loop
overhead is removed.
Second, the indexed binary heap outperforms lazy binary by
$1.4$--$2.2\times$ for $k\geq 8$: at $V=10{,}000$, $k=32$ the indexed
heap takes 1.31\,ms vs.\ lazy binary's 2.83\,ms ($2.2\times$ faster).
