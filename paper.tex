...
systematically quantified as a closed-form empirical model of graph structure.
...
of magnitude ($V\in[500,\,10^6]$; source-vertex CV $\leq 2.5\%$).
...
with the locally tree-like structure underlying the formula's modeling assumptions.
...
\begin{enumerate}
  \item \textbf{RQ1.} What graph property determines skip\_ratio, and can
        it be captured by a closed-form empirical model?
...
deletion overhead as a function of average degree, nor proposes a
closed-form empirical model for the skip-pop fraction.
...
predictive formula.  To the best of our knowledge, no prior work proposes a closed-form
empirical power-law model $\text{skip\_ratio}(k)\approx 1-c/k^{\alpha}$
...
All code, experiment scripts (Python and C++), and raw CSV data are
available at \url{https://github.com/Korean3247/dijkstra-skip-ratio}.
...
\noindent\textbf{Finding~2: $\alpha$ scales with $\log(W_{\max}/W_{\min})$.}
When the \emph{range} is fixed at 100 but $W_{\min}$ varies,
$\alpha$ tracks $\log(W_{\max}/W_{\min})$ rather than either endpoint:
$W_{\min}\!\in\!\{1,10,100\}$ gives $\alpha\!\in\!\{0.272,0.204,0.096\}$.
...
\alpha(k\!=\!128{\to}256)  \approx 0.204, \\
...
\toprule
Graph & $\theta$ & Wrong (\%) & Push reduc. & Time (ms) & Speedup \\
\midrule
...
\toprule
\textbf{Condition} & \textbf{Recomm.} & \textbf{Rationale} \\
\midrule
...
\addlinespace
Float weights, $k > 10$, C++ (supplemental) &
  Consider indexed/4-ary &
  Supplemental evidence (§\ref{sec:heapcompare}): indexed $1.4$--$2.2\times$
  faster, 4-ary $1.4$--$1.6\times$ faster; verify on target platform. \\
\addlinespace
Float weights, $k > 10$, Python &
  heapq (lazy deletion) &
  Indexed and $d$-ary heaps slower than heapq in Python; lazy wins. \\
\addlinespace
Fibonacci heap &
  Avoid in practice &
...
All three lazy-deletion variants (binary, 4-ary, 8-ary) produced
skip\_ratios within $\pm 0.001$ of each other at every $(V,k)$ tested;
representative values at $V=3{,}000$ are $0.242$, $0.438$, $0.573$,
$0.660$, $0.711$, $0.746$ for $k=4,8,16,32,64,128$.
The \emph{indexed} binary heap achieves skip\_ratio$\,=0$ by maintaining
a position array that supports $O(\log V)$ decrease-key operations.
...
from $1.2\times$ at $k=4$ to $1.7\times$ at $k=64$.

\textit{C++ heap comparison.}
We repeated the four-variant comparison in \texttt{dijkstra\_heap\_compare.cpp}
(\texttt{-O2}, Apple M3, directed ER, $m=Vk$ edges).
The findings reverse relative to Python.
First, in C++ the 4-ary lazy heap is consistently \textbf{faster} than
the lazy binary heap by $1.4$--$1.6\times$ ($V=10{,}000$: 0.76\,ms
vs.\ 1.22\,ms at $k=4$; 3.95\,ms vs.\ 5.61\,ms at $k=128$)---the
reduced tree height offsets the wider sift-down scan once the
interpreted-loop overhead is removed.
Second, the indexed binary heap outperforms lazy binary by
$1.4$--$2.2\times$ for $k\geq 8$ ($V=10{,}000$, $k=32$: 1.31\,ms
vs.\ 2.83\,ms, $2.2\times$); this advantage grows with $k$ because
the lazy heap accumulates $O(\text{push\_count})$ entries while the
indexed heap maintains exactly $V$ entries throughout.
In Python the reverse held: heapq's C backend makes each push cheaper
than an interpreted decrease-key lookup.
The C++ results suggest that for high-$k$ production code an indexed
heap or 4-ary lazy heap ($d=4$) is preferable to lazy binary deletion,
even absent a skip-ratio reduction (full data: \texttt{resultsHeapCompare/}).
\emph{Pairing heaps} offer favorable amortized decrease-key and remain
uncharacterized under lazy deletion---an open extension.
...
ran a supplementary experiment (\texttt{source\_sensitivity.py}) sampling
20 uniformly random source vertices across 5 graph instances, for
$V\in\{1{,}000,\,3{,}000,\,10{,}000\}$ and $k\in\{4,8,16,32,64,128\}$
($\mathrm{U}[1,100]$).  The graph model matches \texttt{theory\_probe\_v4.py}
exactly: undirected ER with $m=\lfloor Vk/2\rfloor$ random edges plus a
random spanning tree, so skip\_ratio values are directly comparable to the
main results.
The coefficient of variation of skip\_ratio across sources is at most
$2.5\%$ (at $V=1{,}000$, $k=4$) and falls below $0.5\%$ for $k\geq 64$.
Standard deviation across sources does not exceed $0.010$ at any $(V,k)$
...
lazy deletion overhead in Dijkstra's algorithm as a closed-form
\emph{empirical model} of graph structure, filling a gap in prior
experimental surveys that treat this overhead as a fixed implementation
detail.  Our key findings are:
...
Specifically: use Dial's for bounded integer weights ($W \lesssim 1{,}000$
for $k{=}8$; any $W$ for $k{\geq}32$); use heapq as the portable
float-weight baseline; consider indexed or 4-ary heaps for optimized
C++ implementations at higher $k$ (supplemental evidence, §\ref{sec:heapcompare});
and avoid Fibonacci heaps in practice.
