...
$W_{\min}$ varies, $\alpha$ tracks $\log(W_{\max}/W_{\min})$ rather
than either endpoint alone: $W_{\min}=1\to\alpha=0.272$,
$W_{\min}=10\to\alpha=0.204$, $W_{\min}=100\to\alpha=0.096$.
...
\textit{C++ heap comparison} (\texttt{dijkstra\_\allowbreak heap\_\allowbreak compare.cpp}).
We repeated the four-variant comparison in optimized C++ (\texttt{-O2},
Apple M3, directed ER with $m=Vk$ edges).
The key findings reverse relative to Python.
...
$1.4$--$2.2\times$ for $k\geq 8$: at $V=10{,}000$, $k=32$ the indexed
heap takes 1.31\,ms vs.\ 2.83\,ms for lazy binary ($2.2\times$).
This advantage grows with $k$ because the lazy heap accumulates
$O(\text{push\_count})$ entries (cache misses increase), whereas the
indexed heap maintains exactly $V$ entries throughout.
In Python the reverse held because heapq's C implementation makes each
push cheaper than a decrease-key position lookup in interpreted code.
The C++ results suggest that for high-$k$ graphs in production code, an
indexed heap or a $d$-ary lazy heap ($d=4$) is preferable to binary
lazy deletion, even absent a skip-ratio reduction.
Results are saved to \texttt{resultsHeapCompare/\allowbreak heap\_compare\_cpp\_*.csv}.
\emph{Pairing heaps} offer favorable amortized decrease-key and have not
been characterized under lazy deletion; extending the selection framework
to them remains open.
