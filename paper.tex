...
``push-and-skip'' strategy has never been rigorously quantified as a
function of graph structure.  We identify \emph{skip\_ratio}---the
fraction of heap pops that are immediately discarded as stale---as the
primary hidden cost, and show empirically that it follows a power law
in average degree $k$:
$\text{skip\_ratio}(k)\approx 1 - 0.884/k^{0.262}$ ($R^2{=}0.984$,
$\mathrm{U}[1,100]$ weights), independent of graph size $V$ across
more than three orders of magnitude ($V\in[500,\,10^6]$).
...
strongly associated with $\log(W_{\max}/W_{\min})$ and the fitted $k$-range,
and the local slope converges toward $1/H_k$ as $k\to\infty$---connecting
the empirical formula to the classical harmonic record-minimum bound.
...
Fibonacci heap is $1.5$--$2.3\times$ \emph{slower} than heapq
despite its asymptotic optimality.  The power-law formula generalizes to Barab\'{a}si--Albert
scale-free graphs ($<8\%$ error) but diverges on planar road
networks---consistent with the locally tree-like structure underlying
the formula's derivation.
We conclude with a practitioner-ready data-structure selection
framework grounded in $k$ and weight type.
\end{abstract}
...
\textbf{skip\_ratio}---has never been quantified as a function of graph
topology.  Practitioners accept the overhead as ``negligible'' without
empirical evidence, while theoreticians focus on asymptotic complexity and
ignore constant-factor behavior.  As a result, no principled criterion
...
predictive formula.  To the best of our knowledge, the power-law
characterization $\text{skip\_ratio}(k)\approx 1-c/k^{\alpha}$ and the
data-structure selection framework grounded in it are novel contributions.
...
\]
where the second equality holds because every non-skip pop settles exactly
one vertex, and push\_count $=$ pop\_count (every pushed entry is
eventually popped or abandoned in Dial's).
...
our formula is the first to quantify it precisely and to show that
the power law is the correct functional form in the practically relevant
range $k\in[4,512]$.
...
Table~\ref{tab:vindependence} reports skip\_ratio across \emph{six} orders of
magnitude in $V$ for values of $k\in\{8,32,64\}$, extended to
$V\in\{200{,}000;\,500{,}000;\,1{,}000{,}000\}$ in Experiment~A.
The maximum variation at fixed $k$ is $\Delta\leq 0.013$ across the full
range $V\in[500,\,10^6]$, confirming that skip\_ratio is determined solely
by $k$.  Figure~\ref{fig:vindep} visualizes this: the curves for each $k$
...
orders of magnitude ($V\in[500,\,10^6]$).  Entries marked ``---'' were not measured at that $(V,k)$
combination.}
...
distribution families (Experiments D1--D3 below;
$V=3{,}000$, $k\in\{4,8,16,32,64,128\}$, 30~repetitions each).
...
consistent with the asymptotic skip\_ratio form $1-c/H_k$ (Exp.~D4,
$V=10{,}000$, continuous float weights).}
...
at $W{=}5{,}000$ and $0.66\times$ at $W{=}10{,}000$, with a breakeven
threshold $W^{*}{\approx}4{,}600$ (linear interpolation).  For denser
...
\multicolumn{3}{p{7.3cm}}{\textsuperscript{\dag}\small
  $W^{*}$ depends on $k$: for $k{=}8$, $W^{*}{\approx}4{,}600$;
...
\noindent\textbf{Structures not evaluated.}
This study compares three structures: binary-heap heapq, Fibonacci heap,
and Dial's bucket queue.  Several practically important alternatives
are not evaluated.
\emph{$D$-ary heaps}~\cite{ahuja1993} reduce tree height to increase
cache efficiency; their skip\_ratio profile under lazy deletion mirrors
binary heapq and falls within the same power-law characterization, but
the constant factors differ and merit direct measurement.
\emph{Indexed binary heaps} (with a position array enabling O(1) decrease-key
lookup) eliminate stale entries entirely, avoiding skip\_ratio overhead
at the cost of random-access memory; they may outperform heapq on
sparse float-weight graphs.
\emph{Pairing heaps} offer a simple decrease-key with favorable amortized
cost; their lazy-deletion variant has not been characterized.
Extending the selection framework to these structures is a natural
next step.

\noindent\textbf{Statistical note.}
Main experiments (Sections~\ref{sec:analysis}--\ref{sec:comparison}) use
5--10 repetitions per $(V,k)$ configuration and report means.  Formal
confidence intervals are not provided; however, the observed inter-run
variation is $\leq 0.013$ in skip\_ratio across all $(V,k)$ pairs
(Table~\ref{tab:vindependence}), indicating high stability.
Weight-distribution experiments (Section~\ref{sec:weightdist}) use
30 repetitions ($V=3{,}000$) or 20 repetitions ($V=10{,}000$).
...
We have presented the first systematic empirical characterization of the
lazy deletion overhead in Dijkstra's algorithm as a function of graph
...
our selection framework achieves up to $4\times$ peak speedup.}
Specifically: use Dial's for sparse integer weights, heapq for float
weights, and avoid Fibonacci heaps in practice.
